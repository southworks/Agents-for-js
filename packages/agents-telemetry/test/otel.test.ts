...
// import { _resetForTesting } from '../src/otel.js'
...
describe('otel', () => {
  ...
  describe('@opentelemetry/api installed, no SDK configured', () => {
    ...
    // Mock the entire module before importing
      // Sinon.stub(initOtel, 'getOtelApi').returns(undefined)
      mock.module('../dist/src/initOtel.js', {
        namedExports: {
          getOtelApi: async () => undefined
        }
      })
    })
    ...
    describe('withSpan', () => {
      it('should execute function and return result', async () => {
        // Sinon.stub(otelModule, 'loadOtelApi').resolves(undefined)
...
//         const { withSpan } = await import('../src/otel.js')
...
//         const { startSpan } = await import('../src/otel.js')
...
//         const { getActiveSpan } = await import('../src/otel.js')
...
//         const { withSpan, getActiveSpan } = await import('../src/otel.js')
...
//         const { injectTraceContext } = await import('../src/otel.js')
...
//     beforeEach(() => {
//       // Mock import to return undefined, simulating @opentelemetry/api not installed
//       _resetForTesting({
...
//         const { withSpan, injectTraceContext } = await import('../src/otel.js')
