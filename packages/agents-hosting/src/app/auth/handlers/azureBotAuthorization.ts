...
export class AzureBotAuthorization implements AuthorizationHandler {
    ...
    return trace(SpanNames.AUTHORIZATION_AZURE_BOT_SIGNIN, async (span) => {
      ...
      try {
        ...
        if (active.attemptsLeft <= 0) {
          reason = 'Maximum sign-in attempts exceeded'
        ...
        if (category === Category.SIGNIN) {
          ...
          reason = 'The incoming activity will be revalidated due to a change in the sign-in flow. Usually by user clicking the Consent Cancel button.'
          logger.warn(this.prefix(reason), activity)
        ...
        span.setAttribute('auth.handler.status.reason', reason)
   ...
   */
  private async setToken (storage: HandlerStorage<AzureBotActiveHandler>, context: TurnContext, active?: AzureBotActiveHandler, code?: string, reason: string = ''): Promise<AuthorizationHandlerStatus> {
    const { activity } = context
    ...
    if (!tokenResponse && active && code) {
      reason = `Invalid code entered (${code}). Restarting sign-in flow`
      logger.warn(this.prefix(reason), activity)
    ...
    if (!tokenResponse) {
      reason = 'Cannot find token. Sending sign-in card'
    ...
    reason = 'Successfully acquired token'
    logger.debug(this.prefix(reason), activity)
   ...
   */
  private async handleSignInActivities (context: TurnContext, reason: string): Promise<AuthorizationHandlerStatus> {
    const { activity } = context
    ...
    if (activity.name === 'signin/tokenExchange') {
      ...
      if (!tokenExchangeRequest?.token) {
        reason = 'The Agent received an InvokeActivity that is missing a TokenExchangeInvokeRequest value. This is required to be sent with the InvokeActivity.'
      ...
      if (tokenExchangeInvokeRequest.connectionName !== this._options.name) {
        reason = `The Agent received an InvokeActivity with a TokenExchangeInvokeRequest for a different connection name ('${tokenExchangeInvokeRequest.connectionName}') than expected ('${this._options.name}').`
        await this.sendInvokeResponse<TokenExchangeInvokeResponse>(context, {
          status: 400,
      ...
      if (!token) {
        reason = 'The MS Teams token service didn\'t send back the exchanged token. Waiting for MS Teams to send another signin/tokenExchange request. After multiple failed attempts, the user will be asked to enter the magic code.'
        await this.sendInvokeResponse<TokenExchangeInvokeResponse>(context, {
          status: 412,
          body: { id: tokenExchangeInvokeRequest.id, connectionName: this._options.name!, failureDetail: reason }
        })
      ...
      reason = 'Successfully exchanged token'
      logger.debug(this.prefix(reason))
    ...
    reason = `Unknown sign-in activity name: ${activity.name}`
   ...
   */
  private async codeVerification (storage: HandlerStorage<AzureBotActiveHandler>, context: TurnContext, active?: AzureBotActiveHandler, reason: string = ''): Promise<{ status: AuthorizationHandlerStatus, code?: string }> {
    if (!active) {
    ...
    if (state === 'CancelledByUser') {
      await this.sendInvokeResponse(context, { status: 200 })
      reason = 'Sign-in process was cancelled by the user'
      logger.warn(this.prefix(reason), activity)
      return { status: AuthorizationHandlerStatus.REJECTED }
    }

    if (!state?.match(/^\d{6}$/)) {
      reason = `Invalid magic code entered. Attempts left: ${active.attemptsLeft}`
      logger.warn(this.prefix(reason), activity)
    ...
    await this.sendInvokeResponse(context, { status: 200 })
    reason = 'Code verification successful'
    logger.debug(this.prefix(reason), activity)
