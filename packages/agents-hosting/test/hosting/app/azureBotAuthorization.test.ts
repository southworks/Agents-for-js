...
const createToken = (audience: string) => jwt.sign({ sub: 'user-1' }, 'secret', { audience })
...
describe('AzureBotAuthorization', () => {
  ...
  it('token should exchange a base token for an OBO token when scopes are provided', async () => {
    const baseToken = createToken('api://resource')
  ...
  it('token should return undefined when OBO exchange fails', async () => {
    const baseToken = createToken('api://resource')
    mockClient.getTokenOrSignInResource.resolves({ tokenResponse: { token: baseToken } } as any)
    ...
    const result = await handler.token(context, { scopes: ['scope.read'] })

    assert.deepEqual(result, { token: undefined })
